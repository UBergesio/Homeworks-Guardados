...
function dePalabrasAFrase(palabras) {
   ...
   // Tu código:
   /*var frase = ''
   palabras.forEach(palab => {
      
      if(palab == palabras[palabras.length - 1]){
         (frase = frase + palab + '')
      } 
      (frase = frase + palab + ' ')
   });
...
function agregarNumeros(arrayOfNums) {
   ...
   // Tu código:
   var sumando = 0;
   arrayOfNums.forEach( (num) => { (sumando = sumando + num) } )
   return (sumando)
}
...
function multiplicarArgumentos() {
   ...
   for (let i = 0; i < arguments.length; i++){
       multiplicando = multiplicando * arguments[i]
      }
